 ...
 */
export function estimateSalaryCostFrance(netSalary: number): SalaryEstimation {
...
export const runSimulation = (data: any, scenario: ScenarioType) => {
    ...
    if (data.analysisMode === 'Boutique') {
      ...
      const employeeCosts = (data.employees || []).reduce((acc: number, emp: any) => {
        const est = estimateSalaryCostFrance(emp.salary || 0);
    ...
    return { revenue, totalCosts, netProfit, roi, irr, margin, marketingCosts, logisticCosts, rawCosts, roivaScore, averageStock, stockTurnover, stockDurationDays, stockDurationWeeks, recommendedMinStock, breakEvenPoint };
}
